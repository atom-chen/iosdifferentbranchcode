...
void CCTouchLayer::registerScriptTouchHandler(LUA_FUNCTION nHandler)
{
	m_uLuaTouchHandler = (int)nHandler;
}
...
bool CCTouchLayer::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent)
{
	...
	int id = pTouch->getID();
	//如果是已接受、未释放的touch事件，不应出现重复touch测试，故直接返回
	if((m_uTouchFlags & (1<<id)) != 0)
		return true;
	...
	if(!m_bIsExit){
		...
		if(!m_bTouchHold || ret){
			...
			if(!isTouch){
				if((m_uLuaTouchHandler!=0 || m_uTouchHandler!=NULL) && this->executeTouchHandler(CCTOUCHBEGAN, pTouch)>0){
					ret = true;
...
void CCTouchLayer::ccTouchMoved(CCTouch *pTouch, CCEvent *pEvent)
{
	...
	if(m_pChildTouchMap[id]!=NULL){
		if(m_pChildTouchMap[id]==this){
			this->executeTouchHandler(CCTOUCHMOVED, pTouch);
		}
		else{
...
void CCTouchLayer::ccTouchEnded(CCTouch *pTouch, CCEvent *pEvent)
{
	...
	if(m_pChildTouchMap[id]!=NULL){
		if(m_pChildTouchMap[id]==this){
			this->executeTouchHandler(CCTOUCHENDED, pTouch);
		}
		else{
...
void CCTouchLayer::ccTouchCancelled(CCTouch *pTouch, CCEvent *pEvent)
{
	...
	if(m_pChildTouchMap[id]!=NULL){
		if(m_pChildTouchMap[id]==this){
			this->executeTouchHandler(CCTOUCHCANCELLED, pTouch);
		}
		else{
...
int CCTouchLayer::executeTouchHandler(int nEventType, CCTouch *pTouch)
{
	if(m_bIsExit)
		return 0;
	int ret = 0;
	if(m_uLuaTouchHandler!=0){
		int id = pTouch->getID();
		CCPoint touchPoint = pTouch->getLocation();
		...
		{
			case CCTOUCHBEGAN:
				stack->pushString("began");
				break;
            
			case CCTOUCHMOVED:
				stack->pushString("moved");
				break;
            
			case CCTOUCHENDED:
				stack->pushString("ended");
				break;
            
			case CCTOUCHCANCELLED:
				stack->pushString("cancelled");
				break;
		...
		stack->pushInt(id);
		stack->pushFloat(touchPoint.x);
		stack->pushFloat(touchPoint.y);

		ret = stack->executeFunctionByHandler(m_uLuaTouchHandler, 4);
		stack->clean();
	}
	else if(m_uTouchHandler!=NULL){
		switch (nEventType)
		{
			case CCTOUCHBEGAN:
				if(m_uTouchHandler->ccTouchBegan(pTouch, NULL))
					ret = 1;
				break;
            
			case CCTOUCHMOVED:
				m_uTouchHandler->ccTouchMoved(pTouch, NULL);
				break;
            
			case CCTOUCHENDED:
				m_uTouchHandler->ccTouchEnded(pTouch, NULL);
				break;
            
			case CCTOUCHCANCELLED:
				m_uTouchHandler->ccTouchCancelled(pTouch, NULL);
				break;
