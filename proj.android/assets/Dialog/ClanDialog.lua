...
function ClanDialog:initTabView(tabs, changeTab)
    ...
    if UserData.clan==0 then
        ...
        tabTitles = {"tabMyLeague", "tabSearch", "tabLeagueWar"}
    end
...
local function getBattleMemberDataOver(suc, result, bid)
    ...
    if suc and display.getCurrentScene()==mainScene and not display.isSceneChange then
        ...
        if data.code~=0 then
	        ...
	        mainScene:synData()
            assertNotEqual(UserData.clanBattleId, nil)
...
local function onBattleWithMember(info)
    if info.uid>0 and not UserData.clanBattleId then
        local scene = display.getCurrentScene()
        if BattleLogic.checkBattleEnable(scene, onBattleWithMember, info) then
            UserData.clanBattleId = info.uid
            BattleLogic.enemyMtype = info.mtype
            assertNotEqual(UserData.clanBattleId, nil)
            assertNotEqual(BattleLogic.enemyMtype, nil)
            network.httpRequest("getLeagueMemberData", getBattleMemberDataOver, {single=true,params={uid=UserData.userId, eid=info.uid}, callbackParam=info.binfo[6]})
    	end
    end
end
...
function ClanDialog:loadMembersOver(suc, result, setting)
    ...
    if suc and not self.deleted and (self.memberClan==clan or clan==UserData.clan) then
        ...
        table.sort(data, getSortFunction("lscore", true))
        for i=1, #data do
            data[i].rank = i
        end
        ...
        local function onGetLeagueInfoOver(suc, result)
            if suc and not self.deleted then
                ...
                if info.code==0 then
                    ...
                    table.sort(infos, getSortFunction("lscore", true))
                    for i=1, #infos do
                        infos[i].rank = i
                    end
