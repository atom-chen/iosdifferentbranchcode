        ...
        if item.id==id then
    		...
    		if item.level<=3 then
    			...
    			if General.useGameCenter  then
    				CCNative:reportAchievement("task" .. id .. "." .. item.level, squeeze(100*item.num/item.max, nil, 100))
				end
    		end
            break
        end
...
function Achievements.updateAchieveData(type, key, value)
	...
	if a and a.level<4 then
		...
		if General.useGameCenter and oldValue < a.max and oldValue < a.num then
			CCNative:reportAchievement("task" .. a.id .. "." .. a.level, squeeze(100*a.num/a.max, nil, 100))
		end
