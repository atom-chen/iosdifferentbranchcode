...
function CastleScene:initGround()
    ...
    for i = 0, 11 do
    --[[
        local land = CCExtendNode:create(CCSizeMake(1024, 512), false)
        local temp = UI.createSpriteWithFile("images/background/background" .. i .. ".pvr.ccz")
        temp:setScale(2)
        ...
        land:setScale(2)
        screen.autoSuitable(land, {nodeAnchor=General.anchorLeftBottom, x=(i%4)*1024, y=SIZEY-1024*(1+math.floor(i/4))})
...
function CastleScene:prepare()
    ...
    if not CastleSceneLoadYet then
        ...
        for i=1, 12 do
            loader:addImage("images/background/background" .. (i-1) .. ".pvr.ccz", nil, kCCTexture2DPixelFormat_RGB565)
        end
        loader:addImage("images/build/otherBuilds.pvr.ccz", "images/build/otherBuilds.plist", kCCTexture2DPixelFormat_RGBA4444)
    --else
...
function CastleScene:initBuilds()
    ...
    for i=1, bnum do
        ...
        if build then
            ...
            local setting = {id=id, buildIndex=bcache[bid], initGridX=math.floor(build[2]/10000), initGridY=build[2]%10000, level=build[4], time=build[5]}
            if build[7] and build[7]~="" then
...
function CastleScene:runScaleAndMoveToCenterAction(scale, nodeX, nodeY)
    ...
    local curScale = self.ground:getScaleX()
...
function CastleScene:onTouchMoved(id, x, y)
    ...
    if stouchNum==1 then
        ...
        if state.touchPoint and state.length then
            ...
            self:scaleTo(self.ground:getScaleX()*newLength/state.length, newPoint[1], newPoint[2], true)
        end
        --self.touchType = "scene"
        --state.moving = false
...
function CastleScene:singleTouchMove(ox, oy)
    if self.touchType=="none" then
        ...
        if mov>20 then
            ...
            self.touchType = "scene"
            self.inertia.touchlist.push({time=timer.getTime(), point = self.stateInfo.touchPoint})
            self.stateInfo.touchTime = nil
        end
...
function CastleScene:onTouchEnded(id, x, y)
    ...
    elseif stouchNum == 1 then
        state.touchIds = {}
        state.touchPoints = {}
        if self.ground:getScaleX()>self.scMax then
        ...
        elseif self.touchType=="scene" then
            if self.inertia.touchlist.size() == 2 then
                ...
                if stime < 0.3 then
                    if stime < 0.05 then stime=0.05 end
                    self.inertia.touchlist.clear()
    ...
    stouchNum = #(state.touchIds)
    --I think the backPoint should only be scene's touch
    local backNum = 0
    local backs = {}
    for id, point in pairs(self.backPoints) do
        backNum = backNum+1
        backs[backNum] = {id, point}
    end
    if backNum>0 then
        self.touchType="scene"
        if backNum==1 and stouchNum==0 then
            local p = backs[1]
            self.backPoints[p[1]] = nil
            state.touchPoints[p[1]] = p[2]
            state.touchIds[1] = p[1]
            state.touchPoint = p[2]
        else
            while stouchNum<2 do
                stouchNum=stouchNum+1
                local p = table.remove(backs, 1)
                self.backPoints[p[1]] = nil
                state.touchPoints[p[1]] = p[2]
                state.touchIds[stouchNum] = p[1]
            end
            local id1, id2 = state.touchIds[1], state.touchIds[2]
            state.touchPoint = {(state.touchPoints[id1][1] + state.touchPoints[id2][1])/2, (state.touchPoints[id1][2] + state.touchPoints[id2][2])/2}
            state.length = math.sqrt((state.touchPoints[id1][1] - state.touchPoints[id2][1])^2 + (state.touchPoints[id1][2] - state.touchPoints[id2][2])^2)
        end
    end
end
...
local function loadBattleHistory(isSuc, result)
    if isSuc then
        ...
        if #battles>0 then
            table.sort(battles, getSortFunction("time", true))
...
function OperationScene:initData()
    ...
    for i, build in pairs(initInfo.builds) do
        ...
        EventManager.sendMessage("EVENT_BUILD_UPDATE", {bid=bid, level=build[4]})
    end
...
function OperationScene:synDataOver(suc, result)
...
function OperationScene:synData(isAsyn)

        if self.synOver then
            local deleteIndex = {}
            local buildMap = {}
            for i, binfo in pairs(self.initInfo.builds) do
                buildMap[binfo[1]] = {index=i, info=binfo}
            end
            local delete, update={}, {}
            local newBuilds = {}
            for i, build in pairs(self.builds) do
                if build.deleted then
                    ...
                    if not buildMap[i] then
                        ...
                        table.insert(newBuilds, self.initInfo.builds[buildMap[i].index])
                        buildMap[i] = nil
                    end
                end
            end
            ...
            self.initInfo.builds = newBuilds
            ...
            if CrystalLogic.rewardDay then
                needSyn = true
                params.days = CrystalLogic.rewardDay
                CrystalLogic.rewardDay = nil
            end
            ...
            if needSyn then
                ...
                if not UserData.noPerson then
                    network.httpRequest("synData", self.synDataOver, {isPost=true, params=params, isAsyn=isAsyn}, self)
                else
                    self.synOver = true
                end
            end
        ...
        if params[1]=="build" then
            ...
            for _, build in pairs(self.builds) do
                if build.buildData.bid == bid then
                    ...
                    local sc = self.ground:getScaleX()
                    if bid==TOWN_BID and sc>2*self.scMin then
                        sc = 2*self.scMin
                    end
                    self:runScaleAndMoveToCenterAction(sc, bview:getPositionX(), bview:getPositionY()+bview:getContentSize().height/2)
                    break
                end
            end
        end
    elseif eventType==EventManager.eventType.EVENT_VISIT_USER then
        if not self.visitUid and not network.single then
...
function ZombieScene:addZombieTomb()
    local zombieTombNum = 0
    self.zombieTombs = {}
    for i, build in pairs(self.builds) do
        if build.buildData.bid>=4015 and build.buildData.bid<=4018 then
            zombieTombNum = zombieTombNum + 1
            table.insert(self.zombieTombs, build)
        end
    end
    self.tombNum = 0
    if not ZombieLogic.isGuide then
        local opScene = display.getCurrentScene(1)
        while zombieTombNum<4 do
        
            local bid = 4014+math.random(4)
            local b = Build.create(bid, nil, {type=4})
            local gx, gy 
            while true do
                gx, gy = math.random(40), math.random(40)
                if self.mapGrid:checkGridEmpty(GridKeys.Build, gx, gy, b.buildData.gridSize) then
            ...
            b:addToScene(self, {initGridX=gx, initGridY=gy})
            table.insert(self.builds, b)
            table.insert(self.zombieTombs, b)
            zombieTombNum = zombieTombNum+1
            
            -- add build to operation
            b = Build.create(bid, nil, {type=4,initGridX=gx, initGridY=gy})
            table.insert(opScene.builds, b)
        end
        self.tombNum = zombieTombNum
    end
end
