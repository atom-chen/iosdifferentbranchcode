ChatRoom = {messages={}, globalMsgs={}, beginTime=0, globalBeginTime=0, joinTime=0, chatChannel=0, globalChannel=0}
...
local function onSendMessageGlobal(input)
    if UserData.totalCrystal>=100 then
...
function ChatRoom.checkChat()
    if ChatRoom.chatChannel<1 and UserData.clan>0 then
        ChatRoom.chatChannel = ChatRoom.chatChannel + 1
        local beginTime = (ChatRoom.beginTime or -1)
    	network.httpRequest(network.chatUrl .. "recv", ChatRoom.receiveChat, {params={uid=UserData.userId, cid=UserData.clan, since=beginTime+1}, callbackParam=beginTime, timeout=30})
    end
    if ChatRoom.globalChannel<1 and UserData.totalCrystal>=100 then
        ChatRoom.globalChannel = ChatRoom.globalChannel + 1
        local beginTime = (ChatRoom.globalBeginTime or -1)
        print("begin global chat", beginTime)
    	network.httpRequest(network.chatUrl .. "recv", ChatRoom.receiveGlobalChat, {params={uid=UserData.userId, cid=0, since=beginTime+1}, callbackParam=beginTime, timeout=30})
    end
end
	...
	if suc then
		...
		for i=1, #msgs do
			...
			elseif msg[5]=="sys" then
			    ...
			    local stype = msg[2]
			    -- league battle begin
			    if newMsg.time>UserData.lastSynTime and newMsg.time>ChatRoom.joinTime then
    			    if stype=="lbb" then
...
function ChatRoom.receiveGlobalChat(suc, result, lastTime)
    ChatRoom.globalChannel = ChatRoom.globalChannel - 1
    print("receive global", result, lastTime)
    if ChatRoom.deleted or UserData.totalCrystal<100 then return end
	if suc then
		local msgs = json.decode(result).messages
		for i=1, #msgs do
			local msg = msgs[i]
...
function ChatRoom.onDonate(msg)
    ...
    visitBg:addChild(temp)
    leftNumLabel = temp
    temp = UI.createButton(CCSizeMake(32, 31), UI.closeVisitControl, {callbackParam=bg, image="images/buttonClose.png"})
	...
	for i=1, #msgs do
		if true then
            ...
            if msg.uid~=UserData.userId then
				...
				cell:addChild(temp)
    	        UI.registerVisitIcon(cell, scrollView, ChatRoom.leagueView, msg.uid, temp)
            end
...
function ChatRoom.setVisible(boolValue)
	...
	if boolValue then
		...
		UI.closeVisitControl(ChatRoom.leagueView)
	end
end