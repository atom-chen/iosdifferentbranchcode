...
function ZombieCamp:getExtendInfo()
    if self.buildView then
        ...
        return {soldiers=self.initSetting.soldiers}
    end
end
...
function ZombieCamp:callZombie(ok)
    if ok then
        if CrystalLogic.changeCrystal(-5) then
            UserData.zombieShieldTime = 0
            display.showDialog(ZombieDialog, false)
        end
    else
        display.showDialog(AlertDialog.new(StringManager.getString("alertTitleZombies"),StringManager.getString("alertTextZombies"),{callback=self.callZombie, param=true, crystal=5}, self))
    end
end

function ZombieCamp:addChildMenuButs(buts)
    table.insert(buts, {image="images/zombie.png", text=StringManager.getString("buttonZombie"), callback=self.callZombie, callbackParam=self, extend={imgScale=0.8}})
end
...
function ZombieCamp:getBuildView()
