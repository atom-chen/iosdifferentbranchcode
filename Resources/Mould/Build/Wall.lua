...
function Wall:touchTest(x, y)
    local inNode = isTouchInNode(self.buildView.build, x, y)
    if inNode and self.dirWall[1]~=self.dirWall[2] then
        local point = self.buildView.build:convertToNodeSpace(CCPointMake(x,y))
        local size = self.buildView.build:getContentSize()
        if (self.dirWall[1]==0 and point.x*5>=3*size.width) or (self.dirWall[2]==0 and point.x*5<2*size.width) then
            inNode = false
        end
    end
    return inNode
