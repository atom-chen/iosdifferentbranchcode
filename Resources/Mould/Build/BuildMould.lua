...
function BuildMould:ctor(bid, setting)
    ...
    if self.buildLevel == 0 then
        ...
        if (setting.time or 0) == 0 then
            self.buildData = {bid=bid, level=0, hitPoints=0, gridSize=tempBuildData.gridSize, soldierSpace=tempBuildData.soldierSpace, extendValue1=0, extendValue2=0}
        else
            self.buildData = tempBuildData
        end
    else
...
function BuildMould:onEnterScene()
    ...
    if self.buildLevel>0 and (scene.sceneType==SceneTypes.Operation or (self.supportVisit and scene.sceneType==SceneTypes.Visit)) then
        self:enterOperation()
...
function BuildMould:cancelRepair()
    if self.builder then
        self:releaseBuilder()
        self.buildView:changeUpdateState(false)
...
function BuildMould:checkTouch(x, y)
    if not self.buildView then return false end
    return self.buildView:checkTouch(x,y)
end
...
function BuildMould:getBaseInfo()
    ...
    if self.getExtendInfo then
        local ext = self:getExtendInfo()
        if ext then    baseInfo[7] = json.encode(ext) end
    end
    if self.buildState == BuildStates.STATE_BUILDING then
        baseInfo[5] = timer.getServerTime(self.buildEndTime)
    end
    return baseInfo
end
...
function BuildMould:upgradeOver()
    ...
    self:enterOperation()
    if self.buildView then
        if self.buildView.state.moveState==false then
            self.buildView:resetGrid()
        end
        self.buildView:resetBuildView()
...
function BuildMould:updateZombie(diff)
    if self.builder and self.buildData.bid~=2004 then
        self.buildView:repair(diff)
    end
    if self.updateBattle then
        self:updateBattle(diff)
    end
end
