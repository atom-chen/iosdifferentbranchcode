...
function BuildView:resetBuyingView(isClear)
	...
	if self.buyingView then
		if self.state.moveOk then
			self.buyingView[1]:setSatOffset(0, true)
		else
			self.buyingView[1]:setSatOffset(-100, true)
		end
	end
end
...
function BuildView:damage(value)
	...
	if self.blood then
		self.blood:changeValue(self.hitpoints, false)
	end
	...
	if self.scene.sceneType == SceneTypes.Battle then
	    if not self.buildMould.buildView then
	        self.buildMould.buildView = self
	    end
	    if self.scene and self.scene.builds[self.buildMould.id]~=self.buildMould then
	        self.scene.builds[self.buildMould.id]=self.buildMould
	    end
...
function BuildView:createTimeProcess(processImg, endTime, totalTime)
	...
	timeProcess.view:runAction(CCRepeatForever:create(CCSequence:createWithTwoActions(CCDelayTime:create(0.1), CCCallFunc:create(callfuncUpdate))))
end
...
function BuildView:removeView()
	...
	-- because of the food factory's view is different from the others
	-- the easiest way is to add these 'foolish code'
	if self.buildMould.buildData.bid==2002 then
		self.buildMould.resourceState = nil
	end
	self.buildMould.updateOpened = nil
	self.buildMould = nil
	self.deleted = true
end

function BuildView:repair(diff)
	...
	if not self.deleted and self.blood then
		self.blood:changeValue(self.hitpoints, self.buildMould.builder)
	end
end
...
function BuildView:changeUpdateState(isOpen)
	...
	if not opened and isOpen then
		if scene.sceneType==SceneTypes.Operation or scene.sceneType==SceneTypes.Visit then
			if not self.state.isBuying and (mould.buildState==BuildStates.STATE_BUILDING or (mould.updateOperationLogic and mould.buildState==BuildStates.STATE_FREE and mould.buildLevel>0 and not self.isOperationDestroy and not mould.noOperationUpdate)) then
		...
		elseif scene.sceneType==SceneTypes.Battle or scene.sceneType==SceneTypes.Zombie then
			if mould.updateBattle or (mould.builder and mould.buildData.bid~=2004) then
				if scene.sceneType==SceneTypes.Battle then
					UpdateLogic.addUpdate(mould, mould.updateBattle)
				else
					UpdateLogic.addUpdate(mould, mould.updateZombie)
				end
			end
		end
	elseif opened and isOpen then
		mould.updateDeleted = nil
	elseif opened then
		if scene.sceneType==SceneTypes.Operation or scene.sceneType==SceneTypes.Visit then
			if mould.buildState~=BuildStates.STATE_BUILDING and (not mould.updateOperationLogic or mould.noOperationUpdate) then
				mould.updateDeleted = true
			end
		elseif scene.sceneType==SceneTypes.Battle or scene.sceneType==SceneTypes.Zombie then
			if self.deleted or not mould.updateBattle then
				mould.updateDeleted = true
			end
		end
	end
...
function BuildView:enterOrExit(enter)
    if enter then
		...
		if self.updatePause then return end
        self.buildMould.updateDeleted = nil
        self:changeUpdateState(true)
    else
        self.viewDeleted = true
        if self.updatePause then return end
        self.buildMould.updateDeleted = true
    end
end
		...
		if scene.sceneType == SceneTypes.Operation and self.hitpoints < buildData.hitPoints then
			...
			local function updateBlood()
		    	...
		    	if self.hitpoints<buildData.hitPoints then
	    			...
	    			self.blood.view:stopActionByTag(haction_tag)
	    		end
			end
			self.blood.view:setVisible(true)
			self.blood.view:setOpacity(255)
			self.blood.process:setOpacity(255)
			local haction = CCRepeatForever:create(CCSequence:createWithTwoActions(CCDelayTime:create(0.1), CCCallFunc:create(updateBlood)))
			haction:setTag(haction_tag)
