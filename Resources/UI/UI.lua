...
do
    ...
    local function createBMFontLabel(text, fontName, fontSize, setting)
        ...
        if area.width==0 then
            label = CCLabelBMFont:create(text, fontName, kCCLabelAutomaticWidth, align)
    ...
    local function createTTFLabel(text, fontName, fontSize, setting)
        ...
        local edgeWidth = 0
        if fontName~=General.font5 then
            fontName = General.font5
    ...
    local function createSpecialButton(size, callback, setting)
        ...
        local node = CCExtendNode:create(size, false)
        ...
        local function onTouch(event, id, x, y)
            if event==CCTOUCHBEGAN then
                if not isTouched and (isTouchInNode(node, x, y) or params.parentTouch and isTouchInNode(params.parentTouch, x, y)) then
                    isTouched = true
            ...
            elseif event==CCTOUCHMOVED then
                local eTouched = (isTouchInNode(node, x, y) or params.parentTouch and isTouchInNode(params.parentTouch, x, y))
        ...
        if params.parentTouch then
            local parent = params.parentTouch
    ...
    function UI.createTextInput(baseStr, fontName, fontSize, inputSize, align, limit, priority)
        ...
        local input = CCTextInput:create(baseStr, inputSize, align, fontName, fontSize, limit)
