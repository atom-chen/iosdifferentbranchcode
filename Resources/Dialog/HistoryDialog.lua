...
local function onVideo(info)
    network.httpRequest("getReplay", beginReplay, {single=true, params={uid=UserData.uid, vid=info.videoId}})
end
...
local function onReverge(info)
    if info.id>0 and not HistoryDialog.revergeId then
        ...
        if BattleLogic.checkBattleEnable(scene, onReverge, info) then
            HistoryDialog.revergeId = info.id
            BattleLogic.revergeItem = info
            network.httpRequest("reverge", revergeGetData, {single=true, params={uid=UserData.userId, eid=info.id}})
        end
    end
end
...
local function updateHistoryCell(bg, scrollView, info)
    ...
    for i=1, #items do
        ...
        elseif item[2]==0 then
            ...
            temp = UI.createSpriteWithFile("images/leagueIcon/" .. item[2] .. ".png",CCSizeMake(32, 37))
