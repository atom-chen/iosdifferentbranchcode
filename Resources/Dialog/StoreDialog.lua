...
do
	...
	local function showTreasureTab(param)
		...
		for i=1, #types do	
			...
			local name = StringManager.getString("dataBuildName" .. RESOURCE_BUILD_BIDS[resourceType])
			if num*10/9<max then
				local info = {resource=resourceType, get=math.ceil(max/10), text=StringManager.getFormatString("storeItemResource1", {name=name}), img=prefix .. "1.png"}
				info.cost = CrystalLogic.computeCostByResource(resourceType, info.get)
				table.insert(infos, info)
			end
			if num*2<max then
				local info = {resource=resourceType, get=math.ceil(max/2), text=StringManager.getFormatString("storeItemResource2", {name=name}), img=prefix .. "2.png"}
				info.cost = CrystalLogic.computeCostByResource(resourceType, info.get)
				table.insert(infos, info)
			end
			if num<max then
				local info = {resource=resourceType, get=max-num, text=StringManager.getFormatString("storeItemResource3", {name=name}), img=prefix .. "3.png"}
				info.cost = CrystalLogic.computeCostByResource(resourceType, info.get)
				table.insert(infos, info)
			end
		end
	...
	local function updateMainTabCell(cell, scrollView, info)
        ...
        if info.id==1 then
            if timer.getTime()-UserData.lastOffTime>=86400*7 then
                temp = UI.createSpriteWithFile("images/newNumIcon.png",CCSizeMake(47, 36))
                screen.autoSuitable(temp, {x=9, y=192})
                cell:addChild(temp)
                temp = UI.createLabel(StringManager.getString("labelSaleOff"), General.font1, 18, {colorR = 255, colorG = 255, colorB = 255})
                screen.autoSuitable(temp, {x=33, y=210, nodeAnchor=General.anchorCenter})
                cell:addChild(temp)
            end
		elseif UserData.canBuild and info.id>=2 and info.id<=4 and UserData.canBuild[info.id-1]>0 then
            temp = UI.createSpriteWithFile("images/newNumIcon.png",CCSizeMake(47, 36))
            screen.autoSuitable(temp, {x=9, y=192})
            cell:addChild(temp)
