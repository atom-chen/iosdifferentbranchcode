...
do
    ...
    if director:getRunningScene() then
        director:replaceScene(baseScene)
    else
    ...
    function display.pushNotice(node)
        ...
        local my = node:getContentSize().height * node:getScaleY()
    ...
    function display.popScene(delegate)
        if display.isSceneChange or (#sceneStack)<=1 then return end
        local stackIndex = #sceneStack
        local fromScene = sceneStack[stackIndex]
        sceneStack[stackIndex] = nil
        changeScene(fromScene, sceneStack[stackIndex-1], delegate)
    end
