...
local function updateBattleCell(bg, scrollView, item)
	...
	if item.id>0 then
		...
		elseif item.type=="zombie" then
            ...
            bg:addChild(temp)
        elseif item.type=="clan" then
            temp = UI.createSpriteWithFile("images/leagueIcon/" .. UserData.clanInfo[2] .. ".png",CCSizeMake(54, 62))
            screen.autoSuitable(temp, {x=12, y=12})
            bg:addChild(temp)
		end
...
function BattleMenuLayer:initTips()
    if BattleLogic.isGuide then
        ...
        if not BattleLogic.isReverge and not BattleLogic.isLeagueBattle then
...
function BattleMenuLayer:initLeftTop()
	...
	temp = UI.createLabel(enemyName, General.font6, 22, {colorR = 255, colorG = 255, colorB = 231, lineOffset=-12})
	...
	bg:addChild(temp)
	temp = UI.createLabel(StringManager.getString("labelAvaliable"), General.font2, 13+NORMAL_SIZE_OFF, {colorR = 255, colorG = 255, colorB = 255})
	...
	if BattleLogic.isLeagueBattle then
    	temp = UI.createSpriteWithFile("images/leagueIcon/" .. UserData.clanInfo[2] .. ".png",CCSizeMake(29, 33))
    	screen.autoSuitable(temp, {x=35, y=80})
    	bg:addChild(temp)
    	temp = UI.createLabel(tostring(BattleLogic.enemyMtype*2+1), General.font4, 20, {colorR = 255, colorG = 255, colorB = 255, lineOffset=-12})
    	...
    	screen.autoSuitable(temp, {x=71, y=98, nodeAnchor=General.anchorLeft})
    	bg:addChild(temp)
    	temp = UI.createLabel(StringManager.getString("labelDefeat"), General.font2, 13+NORMAL_SIZE_OFF, {colorR = 255, colorG = 255, colorB = 255})
...
function BattleMenuLayer:endBattle(forceEnd)
	if self.troopsCost then
		...
		if BattleLogic.isLeagueBattle then
		    network.httpRequest("clearBattleState", doNothing, {isPost=true, params={uid=UserData.userId, eid=UserData.enemyId}})
		    BattleLogic.isLeagueBattle = nil
		    BattleLogic.enemyId = nil
...
function BattleMenuLayer:executeSelectItem(touchPoint)
	...
	if not BattleLogic.battleEnd and item then
    	...
    	elseif item.type=="zombie" or item.type=="clan" then
			...
			if self.scene.mapGridView:checkGridEmpty(GridKeys.Build, grid.gridPosX, grid.gridPosY, 2) and self.scene.mapGrid:checkGridEmpty(GridKeys.Build, grid.gridPosX, grid.gridPosY, 2) then
			    if item.type=="zombie" then
			        ...
			        self.hasClan = true
    			    local clanTomb = ClanTomb.new(2, {icon=UserData.clanInfo[2]})
    			    clanTomb:addToScene(self.scene, {initGridX=grid.gridPosX, initGridY=grid.gridPosY})
        			if not self.battleBegin then
        				self:beginBattle()
        			end
    			    clanTomb.buildView.build:setPositionY(26)
        			self.troopsCost = true
        			table.insert(ReplayLogic.cmdList, {timer.getTime()-ReplayLogic.beginTime, "cl", grid.gridPosX, grid.gridPosY, UserData.clanInfo[2]})
        			BattleLogic.clanDeployed = true
        		end
    		else
			    display.pushNotice(UI.createNotice(StringManager.getString("noticeAreaError")))
			    if self.battleBegin then
		...
		if item.num==0 then
			item.view:setSatOffset(-100, true)
...
function BattleMenuLayer:updateOthers()
	...
	if self.soldierEmpty then
	    if not self.emptyCD or self.emptyCD<timer.getTime() then
	        ...
	        if not self.hasZombie and not self.hasClan then
